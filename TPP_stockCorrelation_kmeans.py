...
# Filter the dataset for the selected stocks
    ...
    correlation_matrix_0 = sum(np.outer(daily_returns.iloc[i], daily_returns.iloc[i]) for i in range(n_days))
    rescaling_factor = 1/n_days
    correlation_matrix = correlation_matrix_0 * rescaling_factor
...
s_thresh = sample.sample(correlation_matrix, n_mean, samples, threshold=True)
s_pnr = sample.sample(correlation_matrix, n_mean, samples, threshold=False)
...
def calculate_correlation_matrix_KERNEL(selected_stocks):
    filtered_df = all_stocks_df[all_stocks_df['Name'].isin(selected_stocks)]
    pivot_df = filtered_df.pivot(index='date', columns='Name', values='close')
    pivot_df.replace(0, np.nan, inplace=True)
    daily_returns = pivot_df.pct_change().dropna()
...
correlation_matrix_0 = calculate_correlation_matrix_KERNEL(cluster_0)
correlation_matrix_1 = calculate_correlation_matrix_KERNEL(cluster_1)
