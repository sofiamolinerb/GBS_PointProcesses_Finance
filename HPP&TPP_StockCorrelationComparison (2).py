...
axes[2, 0].bar(range(len(sum_pnr_diff)), sum_pnr_diff, color='orange')
...
axes[2, 1].bar(range(len(sum_pnr_same)), sum_pnr_same, color='orange')
...
axes[1, 0].set_ylabel('Normalized Probability')
axes[1, 0].set_ylim(0, 1.1)  # Normalized probabilities should be between 0 and 1
axes[1, 0].set_title('Threshold detector Normalized Probability Distribution from GBS (Different Sectors)')
...
axes[2, 0].set_xlabel('Mode')
axes[2, 0].set_ylabel('Summed Probability')
axes[2, 0].set_ylim(0, max_count_thres+20) 
axes[2, 0].set_title('Threshold detector Unnormalized Summed Probability Distribution from GBS (Different Sectors)')
...
axes[2, 1].set_xlabel('Mode')
axes[2, 1].set_ylabel('Summed Probability')
axes[2, 1].set_ylim(0, max_count_thres +20) 
axes[2, 1].set_title('Threshold detector Unnormalized Summed Probability Distribution from GBS (Same Sector)')

plt.tight_layout()

# Save the combined figure as a PDF
plt.savefig(r"C:\Users\user\Downloads\norm&unnorm_probability_distribution_thresh.pdf", format='pdf', dpi=300)
...
# Assuming probability_distribution_thresh_diff and probability_distribution_thresh_same are already defined as NumPy arrays
...
# ## We see that what really makes a difference is the 'clustering' in correlation. Since the point is that regardless of how the stocks are more or less correlated overall this point process is good at distinguishing correlations CLUSTERS.
# 
# ### Let us now compare the two sectors energy and technology to see which one shows greater overal correlation
# 
# Since both are sampled with the same number of mean photons, the normalised probability distribution will only be significant the closer the most sampled stocks are from one another, showing a higher clustering. See EnergyvsTech
